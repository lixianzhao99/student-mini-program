...
class StockSelectorV2:
    ...
    def _calculate_beta(self, stock_prices, index_prices, period=60):
        ...
        beta = covariance / (index_variance + 1e-10)
    ...
    def select_best_sector(self, end_date):
        ...
        for index_symbol, index_name in self.config['indices'].items():
            try:
                ...
                momentum_long = (prices[-1] / prices[-61]) - 1 if len(prices) > 61 else momentum_mid
    ...
    def factor_scoring(self, stock_df):
        ...
        stock_df['beta_score'] = 1 - self._normalize(abs(stock_df['beta'] - 1.0))
    ...
    def generate_analysis_report(self, symbol):
        ...
        report = []
        report.append('=' * 60)
        report.append(f'{data["name"]} ({data["symbol"]}) 个股分析报告 V2.0')
        report.append('=' * 60)
        report.append('')
...
def on_schedule(context, bar_list):
    ...
    logger.info('=' * 60)
    logger.info(f'开始执行选股任务 V2.0 {now_str}')
    logger.info('=' * 60)
